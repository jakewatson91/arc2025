...
from models import MultiScalePatchTranslator
...
class PatchTranslatorModel(nn.Module):
    def __init__(self, num_channels=NUM_COLORS):
        super().__init__()
...
model       = MultiScalePatchTranslator(num_channels=NUM_COLORS).to(DEVICE)
...
#     all_predictions[task_id] = translate_single_grid(test_input)
